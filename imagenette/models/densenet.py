...
class DenseBlock(nn.Module):
    def __init__(self, in_channels, k_factor, num_units, bottleneck=False):
        ...
        for i in range(num_units):
            ...
            if bottleneck:
                pointwise = ConvBlock(kernel_size=1, in_channels=in_channels, 
                                      out_channels= 4 * k_factor, padding=0) 
                in_channels = 4 * k_factor 
                
            self.dense_units.append(nn.Sequential(*[
                                                pointwise, 
...
class DenseNet(nn.Sequential):
    def __init__(self, num_units_collection=(6, 12, 24, 16), k_factor=32, transition_factor=0.5,
        ...
        # paper
        # layers = [ConvBlock(in_channels=self.in_channels, out_channels=2 * k_factor, 
        ...
        for num_units in num_units_collection[:-1]:
            ...
            layers.append(nn.Sequential(*[dense_block,
                                          ConvBlock(kernel_size=1, in_channels=in_channels, 
                                                    out_channels=int(in_channels * transition_factor), 
                                                    stride=1, padding=0),
